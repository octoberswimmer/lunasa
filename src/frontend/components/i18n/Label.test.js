...
import { Label, LabelProvider } from "./Label"
...
it("returns the label name if label text cannot be found", () => {
	...
	expect(console.error).toHaveBeenCalledWith('Label not found, "missing_label"')
})
...
it("ignores whitespace around token identifier", () => {
	const wrapper = mount(<Label with={{ token: "replacement" }}>label</Label>, {
		label: "text { token }"
	})
